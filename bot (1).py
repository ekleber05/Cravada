...
async def admin_genpicks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Força geração de picks agora — só admin"""
    if update.effective_user.id not in ADMIN_IDS:
        return
    await update.message.reply_text("⏳ Gerando picks agora...")
    picks = await generate_picks()
    if picks:
        await update.message.reply_text(f"✅ {len(picks)} picks gerados com sucesso!")
    else:
        await update.message.reply_text("⚠️ Nenhum pick gerado — pode não ter jogos hoje ou API indisponível.")


async def admin_updatelesoes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Força atualização da injury list agora — só admin"""
    if update.effective_user.id not in ADMIN_IDS:
        return
    await update.message.reply_text("⏳ Atualizando injury list da NBA...")
    await refresh_injury_list()
    from database import get_stats
    stats = get_stats()
    await update.message.reply_text(
        f"✅ Injury list atualizada!\n🕐 {stats['last_injury_update']}"
    )
